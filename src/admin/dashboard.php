...
<section class="card">
  ...
  <a class="btn" href="<?= $BASE_URL ?>/admin/logout.php">Cerrar sesión</a>
...
<div id="modal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.45); padding:24px; overflow:auto;">
  <div class="card" style="max-width:720px; margin:40px auto;">
    ...
    <form id="postForm">
      ...
      <input class="input" type="number" name="read_time" id="read_time" required><br><br>
...
function renderPosts(posts) {
  ...
  postsWrap.innerHTML = posts.map(p => `
    <article class="post" data-id="${p.id}">
      ...
      <p class="small">${escapeHtml((p.category ?? "").charAt(0).toUpperCase() + (p.category ?? "").slice(1))} • ${escapeHtml(p.created_at)} • ${escapeHtml(p.read_time)} min</p>
  ...
  postsWrap.querySelectorAll(".post").forEach(article => {
    ...
    article.querySelector(".btn-edit").addEventListener("click", () => openModal("edit", post));
